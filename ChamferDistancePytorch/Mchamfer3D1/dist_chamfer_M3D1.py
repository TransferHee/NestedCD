...
chamfer_found = importlib.find_loader("mchamfer_3D1") is not None
if not chamfer_found:
    ## Cool trick from https://github.com/chrdiller
    print("Jitting Chamfer Modified 3D")
    cur_path = os.path.dirname(os.path.abspath(__file__))
    build_path = cur_path.replace('chamferM3D1', 'tmp')
    os.makedirs(build_path, exist_ok=True)

    from torch.utils.cpp_extension import load

    chamfer_3DM1 = load(name="mchamfer_3D1",
                         sources=[
...
class chamfer_3DFunction(Function):
    ...
    def forward(ctx, xyz1, xyz2):
        ...
        chamfer_3DM1.forward(xyz1, xyz2, dist1, dist2, idx1, idx2)
