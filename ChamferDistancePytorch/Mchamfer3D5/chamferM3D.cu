...
__global__ void NmDistanceKernel(int b,int n,const float * xyz,int m,const float * xyz2,
     float * result,int * result_i, int* flag1, int* flag2){
	const int batch=512;
...
__global__ void checkFlag(int b, int n, int* idx1, int* idx2, int* flag1, int* flag2, int offset) {
...
int chamfer_cuda_forward(at::Tensor xyz1, at::Tensor xyz2, at::Tensor dist1, at::Tensor dist2, at::Tensor idx1, at::Tensor idx2){
    ...
    for (int i=0; i<6; i++) {
        NmDistanceKernel<<<dim3(32,16,1),512>>>(b, n, xyz1.data<float>(), m,
                                                xyz2.data<float>(), dist1.data<float>(), idx1.data<int>(),
                                                flag1, flag2);

        NmDistanceKernel<<<dim3(32,16,1),512>>>(b, m, xyz2.data<float>(), n,
                                                xyz1.data<float>(), dist2.data<float>(), idx2.data<int>(),
                                                flag2, flag1);

        checkFlag<<<b, 1024>>>(b, n, idx1.data<int>(), idx2.data<int>(), flag1, flag2, 0);
        checkFlag<<<b, 1024>>>(b, n, idx1.data<int>(), idx2.data<int>(), flag1, flag2, 1024);
        ...
        double per = (float)sum / 2048.0 * 100;
